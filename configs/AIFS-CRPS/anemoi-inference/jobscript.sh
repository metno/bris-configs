...
#SBATCH --output=logs/aifs-anemoi-inference.out
...
LEAD_TIME="${LEAD_TIME:-24h}"
OUTPUT_PATH="${OUTPUT_PATH:-./aifs-ens-crps-1.0.nc}"

srun uv run --locked anemoi-inference run \
  aifs-ens-crps-1.0.yaml \
  "date=${START_DATE}" \
  "lead_time=${LEAD_TIME}" \
  "output.netcdf.path=${OUTPUT_PATH}"
